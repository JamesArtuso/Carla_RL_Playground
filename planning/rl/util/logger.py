...
class Logger:
    ...
    def __init__(self,
        ...
        if IS_MAIN_PROC and not eval_mode:
            ...
            log_exists = osp.exists(self.output_dir)

            if resume and not log_exists:
                raise RuntimeError(f'Resume requested but log does not exist!: {self.output_dir}')
            if not log_exists:
            ...
            self.output_file = open(osp.join(self.output_dir, output_fname), 'a' if resume else 'w')
            atexit.register(self.output_file.close)
            
            print(
                colorize(f"{'Resuming' if resume else 'Logging'} data to {self.output_file.name}",
                         'green',
                         bold=True))
            ...
            if(resume):
                self.log_headers = list(self.load_progress().keys())
        else:
    ...
    def save_state(self, state_dict, itr=None):
        ...
        single state and overwrite it at eatensorboard --logdir=/path/to/your/logs --port 6006ch call with the most recent 
    ...
    def load_progress(self):
        ...
        if not osp.exists(self.output_file_path):
    ...
    def set_steps(self, steps: int):
        ...
        if(self.summary_writer is not None):
            self.summary_writer = SummaryWriter(
                ...
                purge_step=self.steps  # ensures TensorBoard continues from the last step
            )
