from planning.rl.util.torch_util import to_device, to_tensor
import numpy as np
import torch
...
class LSTMGaussianActor(nn.Module):
    ...
    def init_hidden(self, batch_size = 1, device =None):
        if(device is None):
            device = next(self.parameters()).device
        h0 = torch.zeros(1, batch_size, self.h_size, device=device)
        c0 = torch.zeros(1, batch_size, self.h_size, device=device)
        return (h0, c0)
...
class LSTMCritic(nn.Module):
    def __init__(self, obs_dim, hidden_size, activation):
        ...
        # Value decoder
        self.value_decoder = nn.Linear(self.h_size, 1)
    ...
    def init_hidden(self, batch_size, device):
        if(device is None):
            device = next(self.parameters()).device
        h = torch.zeros(1, batch_size, self.h_size, device=device)
        c = torch.zeros(1, batch_size, self.h_size, device=device)
        return (h, c)
