...
from planning.rl.policy.model.mlp_ac import (MLPCategoricalActor, MLPGaussianActor,
                                         mlp)
...
from planning.rl.policy.image_encoder_wrapper import ImageEncoderWrapperMLP, CNN
...
class PPO(Policy):
    def __init__(self,
        ...
        @param obs_type: 0: 4 state 1: BEV
        ...
        if obs_type == 0:
            ...
            if isinstance(env.action_space, gym.spaces.Box):
                actor_backend = MLPGaussianActor(self.obs_dim, self.act_dim,
                                         -self.act_lim, self.act_lim,
                                         hidden_sizes, nn.ReLU)
            elif isinstance(env.action_space, gym.spaces.Discrete):
                actor_backend = MLPCategoricalActor(self.obs_dim, env.action_space.n,
                                            hidden_sizes, nn.ReLU)
            actor_encoder = CNN(self.obs_dim)
            critic_backend = mlp([self.obs_dim] + list(hidden_sizes) + [1], nn.ReLU)
            critic_encoder = CNN(self.obs_dim)
            actor = ImageEncoderWrapperMLP(actor_encoder, actor_backend)
            critic = ImageEncoderWrapperMLP(critic_encoder, critic_backend)
        else:
...
class PPOLagrangian(PPO):
    def __init__(self,
        ...
        if(self.obs_type == 0):
            ...
            qc_backend= mlp([self.obs_dim] + list(self.hidden_sizes) + [1], nn.ReLU)
            self.qc_critic = to_device(ImageEncoderWrapperMLP(qc_encoder, qc_backend))
