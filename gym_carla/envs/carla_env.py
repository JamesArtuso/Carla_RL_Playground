...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    self.camera_res = params['camera_res']
    self.birdeye_res = params['birdeye_res']
    self.no_gui = params['no_gui']
    ...
    observation_space_dict = {
      'camera': spaces.Box(low=0, high=255, shape=(self.camera_res, self.camera_res, 3), dtype=np.uint8),
      'lidar': spaces.Box(low=0, high=255, shape=(self.obs_size, self.obs_size, 3), dtype=np.uint8),
      'birdeye': spaces.Box(low=0, high=255, shape=(self.birdeye_res, self.birdeye_res, 3), dtype=np.uint8),
    ...
    # Lidar sensor
    self.lidar_data = np.zeros((0, 4), dtype=np.float32)
    self.lidar_height = 3#2.1 originally
    self.lidar_trans = carla.Transform(carla.Location(x=0.0, z=self.lidar_height))
    self.lidar_bp = self.world.get_blueprint_library().find('sensor.lidar.ray_cast')
    self.lidar_bp.set_attribute('channels', '32')
    self.lidar_bp.set_attribute('upper_fov', '10.0')
    self.lidar_bp.set_attribute('lower_fov', '-30.0')
    self.lidar_bp.set_attribute('range', '40')

    # Camera sensor
    self.camera_img = np.zeros((self.camera_res, self.camera_res, 3), dtype=np.uint8)
    self.camera_trans = carla.Transform(carla.Location(x=0.8, z=1.7))
    self.camera_bp = self.world.get_blueprint_library().find('sensor.camera.rgb')
    # Modify the attributes of the blueprint to set image resolution and field of view.
    self.camera_bp.set_attribute('image_size_x', str(self.camera_res))
    self.camera_bp.set_attribute('image_size_y', str(self.camera_res))
    ...
    self.settings.no_rendering_mode = params['no_gui']  # speedup
  ...
  def reset(self):    
    ...
    def destroy_sensor(sensor):
        if sensor is not None:
            sensor.stop()
            sensor.destroy()
    destroy_sensor(self.collision_sensor)
    destroy_sensor(self.lidar_sensor)
    destroy_sensor(self.camera_sensor)
    ...
    def get_lidar_data(data):
      ...
      self.lidar_data = points.reshape((-1, 4))
  ...
  def _init_renderer_offscreen(self):
      """Initialize birdeye renderer using a hidden pygame display plus offscreen surface."""
      pygame.init()
      pygame.display.init()

      # Hidden 1x1 display so pygame.Surface.convert() works inside render.py
      pygame.display.set_mode((1, 1), pygame.HIDDEN)

      # Actual offscreen surface used for BEV rendering
      self.display = pygame.Surface((self.display_size, self.display_size))
  ...
  def _get_obs(self):
    ...
    birdeye_disp = pygame.surfarray.array3d(self.display)
    birdeye_disp = birdeye_disp[0:self.display_size, :, :]
    birdeye = display_to_rgb(birdeye_disp, self.birdeye_res)
    ...
    point_cloud = self.lidar_data[:, :3].copy()
    #point_cloud[:, 2] *= -1
    ...
    z_bins = [-10, -self.lidar_height+0.5, 10]
    #z_bins = [-10, -self.lidar_height+0.5, 0.1, 10] #Potentially add color lidar later
    ...
    birdeye_lidar = display_to_rgb(birdeye_disp, self.obs_size)
    if self.display_route:
      wayptimg = (birdeye_lidar[:,:,0] <= 10) * (birdeye_lidar[:,:,1] <= 10) * (birdeye_lidar[:,:,2] >= 240)
    else:
      wayptimg = birdeye_lidar[:,:,0] < 0  # Equal to a zero matrix
    ...
    camera = self.camera_img.astype(np.float64)
